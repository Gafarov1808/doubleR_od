...
DoubleRIteration::DoubleRIteration(std::array<double, 3> L1, std::array<double, 3> L2, std::array<double, 3> L3, 
...
void DoubleRIteration::calc_func(double r1, double r2){
    ...
    if (rho1 < 0 || rho2 < 0 || rho3 < 0){ 
        std::cout << "rho1 = " << rho1 << ", " 
                    << "rho2 = " << rho2 << ", " 
                    << "rho3 = " << rho3 << std::endl;
        std::cout << "Дальность отрицательна! Завершение программы..." << std::endl;
        std::exit(1);
    }
...
Gooding::Gooding(std::array<double, 3> L1, std::array<double, 3> L2, std::array<double, 3> L3, 
                    std::array<double, 3> r_st1, std::array<double, 3> r_st2, 
                    std::array<double, 3> r_st3, double tau1, double tau3 
    ) {
...
Gooding& Gooding::operator=(std::initializer_list<double> list){
    if(list.size() == 6){
        std::array<double, 6> arr;
        std::copy(list.begin(), list.end(), arr.begin());
        state_v = arr;
...
void Gooding::calc_func(double r1, double r2){
...
void Gooding::calc_der(double r1, double r2){
...
void Gooding::solver(double r1, double r2) {
    double dr1 = 1, dr2 = 1;
    double f,g;
    double F_1, F_2;
    double k = 0.2;
...
std::array<double, 3> Gooding::CALCPS(int NHREV, double rho1, double rho3, int NUM) {
    
    std::array<double, 3> R1, R3, N, T, vel;
    double R1_abs, R3_abs, N_abs, T_abs, TH;
    bool sign = true;

    for(size_t i = 0; i < 3; ++i) {
        R1[i] = rho1 * L1[i] + r_st1[i];
        R3[i] = rho3 * L3[i] + r_st3[i];
    }
    R1_abs = sqrt(dot_prod(R1, R1));
    R3_abs = sqrt(dot_prod(R3, R3));

    N = cross_prod(R1, R3);
    N_abs = sqrt(dot_prod(N, N));
    TH = atan2(N_abs, dot_prod(R1, R3));

    if (NHREV % 2 == 1) sign = false;
    TH += NHREV * M_PI;
    auto [v_t1, v_r1, v_t2, v_r2] = Lambert(mu, R1, R3, tau3 - tau1, sign);
    T = cross_prod(N, R1);
    T_abs = sqrt(dot_prod(T, T));

    if (NHREV % 2 == 1) T_abs = -T_abs;
    if (T_abs != 0.0) T_abs = 1 / T_abs;

    for(size_t i = 0; i < 3; i++){
        vel[i] = v_r1 * R1[i] / R1_abs + v_t1 * T[i] / T_abs;
    }

}

std::array<double, 6> Gooding::get_state(){return *state_v;}

void Gooding::print_state() {
    const auto& state = get_state();
    std::cout << "[";
    for(size_t i = 0; i < state.size(); ++i){
        std::cout << std::fixed << std::setprecision(14) << state[i] << (i < state.size()-1 ? ", " : "");
