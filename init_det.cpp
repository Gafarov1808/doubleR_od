...
void GoodingOD::calc_func(double r1, double r2){
...
void GoodingOD::calc_der(double r1, double r2){
...
void GoodingOD::solver(double r1, double r2) {
    double dr1 = 1, dr2 = 1;
    double f,g;
    double F_1, F_2;
    double k = 0.2;
...
std::array<double, 3> GoodingOD::CALCPS(int nhrev, double rho1, double rho3) {
...
std::pair<double, double> GoodingOD::OBS3LS(int nhrev, double rho1, double rho3){
    std::array<double, 3> vec;
    double R, dR, eps = 1e-6, num = 1;

    vec = CALCPS(nhrev, rho1, rho3);
    if (num > 0){
        for(size_t i = 0; i < 3; ++i) {R += L2[i] * vec[i];}
    if (R < 0) dR = eps * (2);
    }
    return {1,1};
}

std::array<double, 6> GoodingOD::get_state(){return *state_v;}

void GoodingOD::print_state() {
    const auto& state = get_state();
    std::cout << "[";
    for(size_t i = 0; i < state.size(); ++i){
        std::cout << std::fixed << std::setprecision(14) << state[i] << (i < state.size()-1 ? ", " : "");
